% Dhruv Balwada
% 18 September 2018 
% This is the starting script of relative dispersion calculations 
% scripts to explore the laws, and calculate relative diffusivity extend
% from.
% run main_obs_model.m before running this
...
for i =1:length(distance_class)
    for j =1:2
            ...
            Obs_disp(j,i) = rel_disp(obs_sep(j,i).sep, ndays, 1);
...
for k =1:2:6
    ...
    h(i)= shadedErrorBar_log(T+0.001, Obsdisp_mean(:,1,k), Obsdisp_cilog(:,:,1,k), ...
                    {'-','linewidth',3,'color',colors(i*2,:)},1)
                hold all
    g(i) = loglog(T+0.001, Moddisp(:,1,k), '--', 'color', colors(i*2,:), 'linewidth', 3)
...
A = legend([h(1).mainLine,h(2).mainLine, h(3).mainLine, g, t3, t1], ...
    {'Obs 10-15km', 'Obs 30-35km', 'Obs 50-55km','Mod 11km', 'Mod 33km', 'Mod 50km','~t^3','~t'}); 
...
for k =1:2:6
    ...
    h(i)= shadedErrorBar_log(T+0.001, Obsdisp_mean(:,2,k), Obsdisp_cilog(:,:,2,k), ...
                    {'-','linewidth',3,'color',colors(i*2,:)},1)
                hold all
    g(i) = loglog(T+0.001, Moddisp(:,2,k), '--', 'color', colors(i*2,:), 'linewidth', 3)
...
for k =1:2:6
    ...
    g(i) = loglog(T, Moddisp_zon(:,1,k), '-', 'color', colors(i*2,:), 'linewidth', 3);
    hold all
    loglog(T, Moddisp_mer(:,1,k), '--', 'color', colors(i*2,:), 'linewidth', 3);
...
for k =1:2:6
    ...
    g(i) = loglog(T, Moddisp_zon(:,2,k), '-', 'color', colors(i*2,:), 'linewidth', 3)
    hold all
    loglog(T, Moddisp_mer(:,2,k), '--', 'color', colors(i*2,:), 'linewidth', 3)
...
for k =1:2:6
    ...
    g(i) = loglog(T, Obsdispzon_mean(:,1,k), '-', 'color', colors(i*2,:), 'linewidth', 3)
    hold all
    loglog(T, Obsdispmer_mean(:,1,k), '--', 'color', colors(i*2,:), 'linewidth', 3)
...
for k =1:2:6
    ...
    g(i) = loglog(T, Obsdispzon_mean(:,2,k), '-', 'color', colors(i*2,:), 'linewidth', 3)
    hold all
    loglog(T, Obsdispmer_mean(:,2,k), '--', 'color', colors(i*2,:), 'linewidth', 3)
