...
function [fsle_zon, fsle_mer] = fsle_zonmer(sep, dist_bin, diff_pres, plevel, flag_interp, flag_1time)
...
for i =1:length(dist_axis)
    ...
    for j = 1:length(sep)
        ...
        id1 = find(sep(j).X(1:end-1)<=dist_bin(i) & sep(j).X(2:end)>=dist_bin(i) & ...
              sep(j).dist(1:end-1)<=1000e3 ...
            & sep(j).dP(1:end-1)<diff_pres & ...
            sep(j).P1(1:end-1)>plevel(1) & sep(j).P1(1:end-1)<plevel(2));
        
        id2 = find(sep(j).X(1:end-1)<=dist_bin(i+1) & sep(j).X(2:end)>=dist_bin(i+1) & ...
             sep(j).dist(1:end-1)<=1000e3 ...
            & sep(j).dP(1:end-1)<diff_pres & ...
        ...
        if ~isempty(id1) && ~isempty(id2)
            ...
            for k = 1:length(id22)
                ...
                if flag_interp==0
                    ...
                    t1 = interp1([sep(j).X(id1(k)) sep(j).X(id1(k)+1)], [id1(k) id1(k)+1], dist_bin(i));
                    t2 = interp1([sep(j).X(id22(k)-1) sep(j).X(id22(k))], [id22(k)-1 id22(k)], dist_bin(i+1));
                end
            ...
            if exist('pair')
                lambda(i).ts_zon = [lambda(i).ts_zon log(dist_bin(i+1)/dist_bin(i))./pair];
                lambda(i).dp_zon = [lambda(i).dp_zon pairdp];
            end
    ...
    for j = 1:length(sep)
        ...
        id1 = find(sep(j).Y(1:end-1)<=dist_bin(i) & sep(j).Y(2:end)>=dist_bin(i) & ...
              sep(j).dist(1:end-1)<=1000e3 ...
            & sep(j).dP(1:end-1)<diff_pres & ...
            sep(j).P1(1:end-1)>plevel(1) & sep(j).P1(1:end-1)<plevel(2));
        
        id2 = find(sep(j).Y(1:end-1)<=dist_bin(i+1) & sep(j).Y(2:end)>=dist_bin(i+1) & ...
             sep(j).dist(1:end-1)<=1000e3 ...
            & sep(j).dP(1:end-1)<diff_pres & ...
            sep(j).P1(1:end-1)>plevel(1) & sep(j).P1(1:end-1)<plevel(2))+1;
        ...
        if ~isempty(id1) && ~isempty(id2)
            ...
            for k = 1:length(id22)
                ...
                if flag_interp==0
                    ...
                    t1 = interp1([sep(j).Y(id1(k)) sep(j).Y(id1(k)+1)], [id1(k) id1(k)+1], dist_bin(i));
                    t2 = interp1([sep(j).Y(id22(k)-1) sep(j).Y(id22(k))], [id22(k)-1 id22(k)], dist_bin(i+1));
                end
            ...
            if exist('pair')
                lambda(i).ts_mer = [lambda(i).ts_mer log(dist_bin(i+1)/dist_bin(i))./pair];
                lambda(i).dp_mer = [lambda(i).dp_mer pairdp];
            end
...
for i =1:length(dist_axis)
    fsle_zon(i) = nanmean(lambda(i).ts_zon);
    fsle_mer(i) = nanmean(lambda(i).ts_mer);
end
